		...
		while True:
		    ...
		    if not os.path.exists(filename):
		        with open(filename, "a") as f:
		            f.write('#timestamp,LO_Frequency,Hit,Temp,Hum \n')
			...
			sht_sensor.route()
			humidity =  0  #sht_sensor.get_hum()
			temperature = 0 #sht_sensor.get_temp()
			...
			if (abs(freq_error) > 1e6):
		        ...
		        with open(filename, "a") as f:
		            f.write("%.1f,%.1f,%s,%.1f,%.1f\n" % (time.time(), current_freq, freq_corr, temperature, humidity))
